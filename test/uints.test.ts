...
describe('uints tests', () => {
  ...
  test('delta x flow', async () => {
    const uints = await deployUints(sender)
    const clamm = await deployCLAMM(sender)
    const liquidityScale = 10n ** 5n
    const sqrtPriceScale = 10n ** 24n
    const sqrtPriceA = 234878324943782000000000000n
    const sqrtPriceB = 87854456421658000000000000n
    const liquidity = 983983249092n

    const deltaSqrtPrice = sqrtPriceB > sqrtPriceA ? sqrtPriceB - sqrtPriceA : sqrtPriceA - sqrtPriceB

    const nominator = (
      await uints.contractInstance.methods.bigMulDiv256({
        args: { a: deltaSqrtPrice, b: liquidity, bDenominator: liquidityScale }
      })
    ).returns

    {
      const denominator = (
        await clamm.contractInstance.methods.mul({
          args: { l: sqrtPriceA, r: sqrtPriceB, rScale: 24n }
        })
      ).returns

      let deltaX = (await uints.contractInstance.methods.bigMul512({ args: { a: nominator, b: sqrtPriceScale } }))
        .returns
      console.log(deltaX)
      deltaX = (await uints.contractInstance.methods.bigAdd({ args: { a: deltaX, b: denominator - 1n } })).returns
      console.log(deltaX)
      deltaX = (
        await uints.contractInstance.methods.bigDiv512({ args: { a: deltaX, b: denominator, bDenominator: 1n } })
      ).returns
      console.log(deltaX)
      // expected: 70108199533068819376891841728
      // received: 70108199533068819376891841728
      deltaX = (await uints.contractInstance.methods.bigAdd({ args: { a: deltaX, b: sqrtPriceScale - 1n } })).returns
      console.log(deltaX)
      const result = (
        await uints.contractInstance.methods.knuthDivision({
          args: { dividend: deltaX, divisor: sqrtPriceScale, divisorDenominator: 1n }
        })
      ).returns

      const bigdivupres = (
        await uints.contractInstance.methods.bigDivUp512({
          args: { a: deltaX, b: sqrtPriceScale, bDenominator: 1n }
        })
      ).returns
      console.log(result)
      console.log(bigdivupres)
    }
    {
      console.log('Rounding up = false')
      const denominator = (
        await clamm.contractInstance.methods.mulUp({
          args: { l: sqrtPriceA, r: sqrtPriceB, rScale: 24n }
        })
      ).returns
      let deltaX = (await uints.contractInstance.methods.bigMul512({ args: { a: nominator, b: sqrtPriceScale } }))
      ...
      console.log(deltaX)
      let finaldeltax = (
        await uints.contractInstance.methods.bigDiv512({ args: { a: deltaX, b: sqrtPriceScale, bDenominator: 1n } })
      ).returns
      const result = (
        await uints.contractInstance.methods.knuthDivision({
          args: { dividend: deltaX, divisor: sqrtPriceScale, divisorDenominator: 1n }
  ...
  //       // received: 486326774796728020778998137036489212983733935595690368965721852833235144487738
  //       expect(result).toStrictEqual({
  //         higher: 4n,
